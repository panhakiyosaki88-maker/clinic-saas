...
export function MedicineForm({ medicine }: { medicine?: Medicine }) {
  const t = useTranslations("pharmacy.form");
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      ...
      <div className="space-y-2 rounded-md border border-[var(--border)] p-3">
        ...
        <Input
          ...
          readOnly={autoSku}
          placeholder={autoSku ? t("generatedPlaceholder") : t("enterSku")}
        ...
        {autoSku ? (
          <p className="text-xs text-[var(--muted-foreground)]">
            {baseHint
              ? isEdit
                ? t("skuHintFormatEdit", { base: baseHint })
                : t("skuHintFormat", { base: baseHint })
              : t("skuHintEmpty")}
          </p>
        ) : (
          <p className="text-xs text-[var(--muted-foreground)]">{t("skuHintManual")}</p>
        )}
      </div>
      ...
      <div className="flex gap-2">
        <Button type="submit" disabled={pending}>{pending ? t("saving") : isEdit ? t("save") : t("add")}</Button>
