          ...
          <ul className="divide-y divide-[var(--border)]">
            {services.map((s) => (
              <StudyRow
                ...
                onSave={(input) =>
                  startTransition(async () => {
                    const res = await updateImagingService(s.id, input);
                    if (!res.ok) return setError(res.error);
                    setError(null);
                    setEditingId(null);
                    router.refresh();
...
function StudyRow({
      ...
      <li className="flex items-center justify-between gap-3 py-2 text-sm">
        <div className="min-w-0">
          ...
          <span className="ml-2 text-xs text-[var(--muted-foreground)]">
            {[service.category_id ? catName.get(service.category_id) : null, service.modality].filter(Boolean).join(" · ")}
          </span>
        </div>
  ...
  return (
    <li className="py-2">
      ...
      >
        <div className="space-y-1.5">
          <Label htmlFor={`name-${service.id}`}>{t("studyName")}</Label>
        ...
        <div className="space-y-1.5">
          <Label htmlFor={`mod-${service.id}`}>{t("modality")}</Label>
          <select id={`mod-${service.id}`} name="modality" className={selectClass} defaultValue={service.modality ?? ""}>
            <option value="">—</option>
            {IMAGING_MODALITIES.map((m) => <option key={m} value={m}>{m}</option>)}
          </select>
        </div>
