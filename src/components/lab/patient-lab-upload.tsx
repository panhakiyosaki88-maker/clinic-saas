...
export function PatientLabUpload({
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (requestIds.length === 0) {
      setError("No active tests to attach the result to.");
      return;
    }
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-3">
      ...
      {done && <p className="text-sm text-emerald-600 dark:text-emerald-400">Result uploaded.</p>}
      <Button type="submit" disabled={pending}>{pending ? "Uploading…" : "Upload result"}</Button>
    </form>
  );
}
