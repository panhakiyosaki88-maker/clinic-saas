...
function dayLabel(iso: string): string {
  ...
  return isToday ? `Today ${timeLabel(iso)}` : d.toLocaleDateString(undefined, { month: "short", day: "numeric" });
}
...
export function PatientIntelligence({
  ...
  return (
    <WidgetCard title="Patient Insights" action={{ href: "/patients", label: "All patients" }} bodyClassName="p-5 space-y-4">
      ...
      <div>
        <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold text-slate-500 dark:text-slate-400">
          <ShieldAlert className="size-3.5 text-rose-500" /> High-risk patients
        ...
        {highRisk.rows.length === 0 ? (
          <p className="text-xs text-slate-400">No high-risk patients flagged.</p>
        ) : (
