...
export async function RevenueAnalytics({
  ...
  return (
    <WidgetCard title={t("widget.financialCenter")} action={{ href: "/reports", label: t("action.reports") }}>
      {month.total === 0 && todayTotal === 0 && billing.invoicedTotal === 0 ? (
        <EmptyState icon={DollarSign} title={t("empty.noRevenue.title")} hint={t("empty.noRevenue.hint")} />
      ) : (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-2">
            <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/50">
              <p className="text-xs text-slate-400">{t("labels.today")}</p>
              <p className="text-lg font-bold text-slate-900 dark:text-white">{fmt(todayTotal)}</p>
            </div>
            <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/50">
              <p className="text-xs text-slate-400">{t("labels.thisWeek")}</p>
              <p className="text-lg font-bold text-slate-900 dark:text-white">{fmt(weekTotal)}</p>
            </div>
            <div className="rounded-xl bg-emerald-50 p-3 dark:bg-emerald-500/10">
              <p className="text-xs text-slate-400">{t("labels.thisMonth")}</p>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="rounded-xl border border-slate-200 p-3 dark:border-slate-800">
              <p className="text-xs text-slate-400">{t("labels.collectionRate")}</p>
            ...
            <div className="rounded-xl border border-slate-200 p-3 dark:border-slate-800">
              <p className="text-xs text-slate-400">{t("labels.avgPerPatient")}</p>
          ...
          {month.byMethod.length > 0 && (
            <div className="space-y-2">
              <p className="text-xs font-medium text-slate-400">{t("labels.byMethod")}</p>
