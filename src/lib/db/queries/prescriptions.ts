...
const LIST_SELECT = `*, patients ( full_name, khmer_name, patient_number ), doctors ( full_name, avatar_path ), prescription_items ( id )`;
...
export async function getPrescription(id: string): Promise<PrescriptionDetail | null> {
  ...
  const { data, error } = await supabase
    .from("prescriptions")
    .select(
      `*, patients ( full_name, khmer_name, patient_number ), doctors ( full_name, avatar_path ),
       clinics ( name ),
       prescription_items ( * )`
    )
    .eq("id", id)
